...
public class QuestionVO implements Serializable {
    ...
    public static Question voToObj(QuestionVO questionVO) {
        ...
        if( tagList != null ) {
            question.setTags(JSONUtil.toJsonStr(tagList));// json转字符串
        }
    ...
    public static QuestionVO objToVo(Question question) {
        ...
        List<String> tagList  = JSONUtil.toList(question.getTags(),String.class);// 将json转为String
        questionVO.setTags(tagList);
        String judgeConfigStr = question.getJudgeConfig();
        questionVO.setJudgeConfig(JSONUtil.toBean(judgeConfigStr, JudgeConfig.class));// 将字符串转化为 JudgeConfig 类型
        return questionVO;
    }
